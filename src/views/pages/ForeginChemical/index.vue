...
import {
  getForeignChemical,
  deleteForeignChemical,
  modifyForeignChemical,
  addForeignChemical
} from '@/api/foreignChemical'
import ModifyDialog from '@/components/SimpleDialog'
import SimpleTable from '@/components/Table/SimpleTable'
export default {
  name: 'ForeignChemical',
  components: {
    SimpleTable,
    ModifyDialog
  },
  data() {
    return {
      ...
      formItems: [
        ...
        { label: '武器种类', prop: 'weaponTypes', input: true, type: 'text' },
        { label: '部署位置', prop: 'deployAddress', input: true, type: 'text' },
        {
          label: '对我国威胁影响',
          prop: 'influence',
          input: true,
      ...
      diaLogformRules: {
        ...
        weaponTypes: [
          { required: true, message: '请输入武器种类', trigger: 'blur' }
        ],
        deployAddress: [
          { required: true, message: '请输入部署位置', trigger: 'blur' }
        ],
        influence: [
          { required: true, message: '请输入对我国威胁影响', trigger: 'blur' }
        ]
      },
  ...
  methods: {
    ...
    onGetTableList() {
      ...
      getForeignChemical(this.listQuery).then(res => {
    ...
    onDelete(index, row) {
      this.$confirm('删除无法恢复，确认删除?', '提示', {
        ...
        .then(() => {
          ...
          deleteForeignChemical({ ids: JSON.stringify(id) }).then(res => {
            // console.log(res)
            this.onGetTableList()
            this.$message({
              type: 'success',
    ...
    onConfirm(formData) {
      if (this.isModify) {
        modifyForeignChemical(formData).then(res => {
        ...
        addForeignChemical(formData).then(res => {
